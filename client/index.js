...
function loadInterviewTable(data) {
    ...
    data.forEach(function ({ id, email1, email2, startTime, endTime, resume }) {
        ...
        tableHtml += `<td><a href=${resume}>Resume</a></td>`
        tableHtml += "</tr>";
    });
...
function insertRowIntoInterviewTable(data) {
    ...
    tableHtml += `<td><button class="edit-row-btn" data-id=${dataToStore}>Edit</td>`;
    tableHtml += `<td><a href=${data.resume}>Resume</a></td>`
    tableHtml += "</tr>";
