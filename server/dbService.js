...
class DbService {
    ...
    async insertInterview(email1, email2, startTime, endTime, resume) {
        try {
            ...
            if (check1 > 0) {
                ...
                const insertId = await new Promise((resolve, reject) => {
                    const query = "INSERT INTO interviews (email1, email2, startTime, endTime, resume) VALUES (?,?,?,?,?);";

                    connection.query(query, [email1, email2, start, end, resume], (err, result) => {
                        if (err) reject(new Error(err.message));
                        resolve(result.insertId);
                    })
                });
                const ms = mailService.getMailServiceInstance();
                ms.schedule(email1, email2, startTime, endTime, resume);
                return {
                    id: insertId,
                    email1: email1,
                    email2: email2,
